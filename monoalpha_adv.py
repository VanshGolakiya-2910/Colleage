...
def parse_key(user_input):
    key_pairs = user_input.split(',')
    key = []
    for pair in key_pairs:
        lower, upper = pair.split(':')
        key.append({lower: upper})
    return key

def get_key():
    choice = input("Do you want to provide a predefined key? (y/n): ").strip().lower()
    if choice == 'y':
        user_input = input("Enter the key pairs in the format 'a:A,b:B,...,z:Z' for monoalphabetic key: ")
        return parse_key(user_input)
    else:
        return generate_key()
